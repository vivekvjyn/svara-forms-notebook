...
def calculate_transition_matrix(sequences, window=2):
    transitions = defaultdict(lambda: defaultdict(int))
    for seq in sequences:
        windowed = extract_window_around_capital(seq, window)
        if windowed:
            for i in range(len(windowed) - 1):
                transitions[windowed[i]][windowed[i + 1]] += 1

    # Convert counts to probabilities
    transition_matrix = {char: {next_char: count / sum(next_counts.values())
                                for next_char, count in next_counts.items()}
                         for char, next_counts in transitions.items()}
    return transition_matrix



def extract_window_around_capital(seq, window=2):
    capital_index = next((i for i, c in enumerate(seq) if c.isupper()), None)
    if capital_index is not None:
        start = max(0, capital_index - window)
        end = min(len(seq), capital_index + window + 1)
...
def prec_permutation_test(group, all_groups, k, num_permutations=1000):
    ...
    alpha_expected = [expected_counts.get(seq, 0) + 1 for seq in all_sequences]

    res = permutation_test(observed_counts, expected_counts, num_permutations=1000)
    # chi2
    #res = permutation_test(observed_counts, expected_counts, num_permutations=num_permutations)
    ##_, p_value, effect_size = chi_squared_test(observed_counts, expected_counts)
    #res['n'] = len(sc)
